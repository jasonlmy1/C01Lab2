...
app.patch("/editNote/:noteId", express.json(), async (req, res) => {
  try {
    // Basic param checking
    const noteId = req.params.noteId;
    if (!ObjectId.isValid(noteId) || (!req.body.title && !req.body.content)) {
      return res
        .status(400)
        .json({
          error:
            "Bad request in relation to the :noteId URL parameter, or the bodyparams",
        });
    }
...
app.get("/getAllNotes", express.json(), async (req, res) => {
  try {
    //no need to check id, as we are goin through all of em
    // Verify the JWT from the request headers
    const token = req.headers.authorization.split(" ")[1];
    jwt.verify(token, "secret-key", async (err, decoded) => {
      if (err) {
      ...
      // Find all notes belonging to given User
      const collection = db.collection(COLLECTIONS.notes);
      ...
      if (allnotes.length === 0) {
        ...
        res.status(200).json({ response: allnotes });
      }
    });
  } catch (error) {
    res.status(500).json({ error: error.message });
